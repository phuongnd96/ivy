...
func GetGKEClientsetWithTimeout(projectID string, gkeClusterName string, gkeLocation string) (*rest.Config, error) {
	ctx, cancel := context.WithTimeout(context.Background(), viper.GetDuration("GlobalResourceTimeOut")*time.Minute)
	defer cancel()
	remoteRestConfig, err := GetGKEClientset(ctx, projectID, gkeClusterName, gkeLocation)
	return remoteRestConfig, err
}

// GetGKEClientset return a *rest.Config can be used to connect to a kubernetes
func GetGKEClientset(ctx context.Context, projectID string, gkeClusterName string, gkeLocation string) (*rest.Config, error) {
	var err error
